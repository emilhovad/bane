...
#added Emil Hovad 
    # yolo format is the interesting format.
def draw_yolo_bboxes_from_file(tmp_img, annotation_paths, width, height):
    global img_objects
    img_objects = []
    #works
    ...
    if os.path.isfile(ann_path):
        ...
        for line in lines:
            ...
            #int(float(line_vec[1])*width), int(float(line_vec[2])*height), int(float(line_vec[3])*width), int(float(line_vec[4])*height)
            
            xmin, ymin, xmax, ymax = yolo_to_voc(float(line_vec[1]), float(line_vec[2]), float(line_vec[3]), float(line_vec[4]), width, height)
            
            class_name, class_index = CLASS_LIST[int(line_vec[0])], int(line_vec[0])
            #get_xml_object_data(obj)
...
while True:
    ...
    # this is only for VOC
    if False:
        tmp_img = draw_bboxes_from_file(tmp_img, annotation_paths, width, height)
    
    #DRAW YOLO BOX  
    if True:
        tmp_img = draw_yolo_bboxes_from_file(tmp_img, annotation_paths, width, height)
