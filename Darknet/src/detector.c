...
void train_detector(char *datacfg, char *cfgfile, char *weightfile, int *gpus, int ngpus, int clear, int dont_show)
{
    ...
    while(get_current_batch(net) < net.max_batches){
	    ...
	    printf("i = %d i%500 = %d i%1000 = %d\n", i, i%500, i%1000);
            if((i <= 5000 && i % 100 == 0)) {
                sprintf(buff, "%s/%s_%d.weights", backup_directory, base, i, avg_loss);
                save_weights(net, buff);
                //sprintf(buff, "%s/%s.backup", backup_directory, base);
                //save_weights(net, buff);
            }else if(i < 20000 && i % 500 == 0) {
                sprintf(buff, "%s/%s_%d.weights", backup_directory, base, i, avg_loss);
                save_weights(net, buff);
            }else if(i > 20000 && i % 1000 == 0) {
                sprintf(buff, "%s/%s_%d.weights", backup_directory, base, i, avg_loss);
                save_weights(net, buff);
            }
...
void test_detector(char *datacfg, char *cfgfile, char *weightfile, char *filename, float thresh,
                   float hier_thresh, int dont_show, int dont_save_image, int ext_output, int save_labels)
{
    list *options = read_data_cfg(datacfg);
    char *name_list = option_find_str(options, "names", "data/names.list");
    int names_size = 0;
    ...
    while(1){
        if(filename){
            ...
            //printf("filePath new tmp: %s \n", tmp);
            char *token = strtok(tmp, "/");
            //printf("filePath new tokenfirst: %s \n", token);
            while(token) {
                //puts(token);
                strncpy(backupFileName, token, 256);
                token = strtok(NULL, "/");
                //printf("filePath new token while: %s \n",token);
            }
            printf("filePath new1: %s \n", backupFileName);
            //strtok(backupFileName, ".jpg");
            backupFileName[strlen(backupFileName) - 4] = 0;
            printf("filePath new2: %s \n",backupFileName);
        }
        //strncpy(backupFileName, filename, 256);
        //token = strtok(backupFileName, "/");
        //printf("names: %s",token);
        printf("Image path %s \n",input);
        ...
        //save_image(im, "predictions"); //filename
        //save_image(im, backupFileName);
        //save_image(im, backupFileName);
        if (!dont_show) {
        ...
        if(save_labels)
        {
            ...
            //char *tmp = ;
            char *labelpathnew = (char *) malloc(5 + strlen(labelpath)+ strlen(backupFileName) );
            strcpy(labelpathnew, "output/Labels/");
            strcat(labelpathnew, backupFileName);
            strcat(labelpathnew, ".txt");
            //printf("labelpath: %s \n",labelpath);
            //printf("labelpathnew: %s \n",labelpathnew);
            FILE* fw = fopen(labelpathnew, "wb");
            int i;
            for (i = 0; i < nboxes; ++i) {
                char buff[1024];
                int class_id = -1;
...
void validate_detector_map_Multiple(char *datacfg, char *cfg, char *filename, float thresh)
{
    char buff[256];
    char *weights = buff;
    char backupbuff[256];
    char *backupweights = backupbuff;
    time_t start = time(0);
    while(1){
        if(filename){
            strncpy(weights, filename, 256);
            if(strlen(weights) > 0)
                if (weights[strlen(weights) - 1] == 0x0d) weights[strlen(weights) - 1] = 0;
        } else {
            //printf("Enter weights Path: ");
            fflush(stdout);
            weights = fgets(weights, 256, stdin);
            if(!weights) return;
            strtok(weights, "\n");
            
            char bufftmp[256];
            char *tmp = bufftmp;
            strncpy(tmp, weights, 256);
            char *token = strtok(tmp, "_");
            while(token) {
                //puts(token);
                strncpy(backupweights, token, 256);
                token = strtok(NULL, "_");
                //printf("filePath new: %s \n",token);
            }
            
            strtok(backupweights, ".weights");
            printf("%s: \n",backupweights);
        }
        if(weights)
        if(strlen(weights) > 0)
            if (weights[strlen(weights) - 1] == 0x0d) weights[strlen(weights) - 1] = 0;
        //printf("what is weights: %s", weights);
        
        validate_detector_map(datacfg, cfg, weights, thresh);

        if (filename) break;
    }
    int num_seconds = (double)(time(0) - start);
    int hours = num_seconds / (60 * 60);
    num_seconds = num_seconds - hours * (60 * 60);
    int minutes = num_seconds / 60;
    num_seconds = num_seconds - minutes * 60;
    
    fprintf(stderr, "Total Time: %f Seconds\n HH-mm-ss %f-%f-%f", (double)(time(0) - start), hours, minutes, num_seconds);
...
void run_detector(int argc, char **argv)
{
    ...
    int dont_save_image = find_arg(argc, argv, "-dont_save_image");
    ...
    if(0==strcmp(argv[2], "test")) test_detector(datacfg, cfg, weights, filename, thresh, hier_thresh, dont_show, dont_save_image, ext_output, save_labels);
    else if(0==strcmp(argv[2], "train")) train_detector(datacfg, cfg, weights, gpus, ngpus, clear, dont_show);
    else if(0==strcmp(argv[2], "valid")) validate_detector(datacfg, cfg, weights, outfile);
    else if(0==strcmp(argv[2], "recall")) validate_detector_recall(datacfg, cfg, weights);
    else if(0==strcmp(argv[2], "map")) validate_detector_map(datacfg, cfg, weights, thresh);
    else if(0==strcmp(argv[2], "mapMultiple")) validate_detector_map_Multiple(datacfg, cfg, filename, thresh);
